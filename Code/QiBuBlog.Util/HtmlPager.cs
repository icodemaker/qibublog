...
namespace QiBuBlog.Util
{
    ...
    public class HtmlPager
    {
        ...
        public string GenerateCode(ushort pageCount, int currentPage)
        {
            if (pageCount <= 1)
            {
                return string.Empty;
            ...
            var link = string.Format(
                string.IsNullOrEmpty(HrefPattern) ? "{0}?{1}{2}" : HrefPattern,
                BaseUrl, result.ToString(), "{0}"
            );
            result.Clear();
            ...
            if (currentPage > 1)
            {
                result.Append(
                    string.Format(
                        PageUpDownTpl, new object[] { "pager-prev", "上一页", string.Format(link, (currentPage - 1).ToString()) }
                    )
                );
            }
            ...
            else
            {
                result.Append(
                    string.Format(
                        PageLinkTpl, new object[] { string.Empty, "1", string.Format(link, "1") }
                    )
                );
                ...
                if (temp < currentPage)
                {
                    ...
                    for (var i = temp; i < currentPage; i++)
                    {
                        result.Append(
                            string.Format(
                                PageLinkTpl, new object[] { string.Empty, i.ToString(), string.Format(link, i.ToString()) }
                            )
                        );
                    }
                }

                result.Append(string.Format(PageTextTpl, "pager-current", currentPage.ToString()));
            }
            ...
            if (temp > currentPage)
            {
                ...
                {
                    result.Append(
                        string.Format(PageLinkTpl, new object[] { string.Empty, pageCount.ToString(), string.Format(link, pageCount.ToString()) }
                        )
                    );
                }
            }
            ...
            if (currentPage != pageCount)
            {
                result.Append(string.Format(
                        PageUpDownTpl, new object[] { "pager-next", "下一页", string.Format(link, (currentPage + 1).ToString()) }
                    )
                );
            }
